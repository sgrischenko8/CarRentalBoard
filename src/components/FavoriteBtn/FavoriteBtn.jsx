...
export const FavoriteBtn = ({ fill, id }) => {
  ...
  function toggleFavor() {
    if (favorites.length > 0 && favorites.includes(id)) {
      const newArray = favorites.filter((el) => el !== id);
      return dispatch(setFavorites(newArray));
    }
  ...
  return (
    ...
    >
      ...
      >
        <path
          fill={favorites?.includes(id) ? 'blue' : 'transparent'}
          ...
          stroke={favorites?.includes(id) ? 'blue' : '#ffffff80'}
          strokeOpacity="0.8"
          strokeWidth="1.5"
          strokeLinecap="round"
